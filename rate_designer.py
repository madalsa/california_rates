...
def design_rate(fixed_pct_td=0, remove_wildfire=False, roe_reduction=0,
    ...
    discounts applied, but WITHOUT baseline credits or fixed charges).

    bl_total is the aggregate baseline credits across all buildings (weighted).
    Since designed scenario bills now INCLUDE baseline credits, the scaling
    formula is: scaling = (R_vol + BL_total) / R_gross_vol.
    This ensures revenue neutrality: scaling × R_gross_vol - BL_total + R_fixed = R_target.
    ...
    bl_total : float, optional
        Aggregate baseline credits (weighted). Added to R_vol in scaling
        formula since designed bills now include baseline credits.
        Default 0 (legacy behavior).
    sample_n_care : int
    ...
    # Designed bills = sum(load × rate × scaling) × care_factor - BL_credits.
    # Revenue neutrality requires: scaling × R_gross_vol - BL_total + R_fixed = R_target
    # Therefore: scaling = (R_vol + BL_total) / R_gross_vol

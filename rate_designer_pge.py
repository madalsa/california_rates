...
# β_wf: share of wildfire costs that are T&D-related
# β_roe: T&D rate base / total rate base (ROE earned on T&D assets)
TD_RATEBASE_SHARE = (25_347_030 + 11_600_356) / 41_987_991  # 88.0% for PGE
...
def design_rate(fixed_pct_td=0, remove_wildfire=False, roe_reduction=0,
    ...
    # Remove wildfire and ROE costs from revenue requirement
    ...
    # Wildfire and ROE are largely embedded in T&D. When removed, the T&D
    # cost base shrinks, so fixed charges should reflect the reduced T&D.
    ...
    # Scale TOU rates accounting for constant baseline credit.
    # Designed bill = s × gross_vol - BL + FC. Summing over all buildings:
    #   R_collected = s × (R_sample + BL_total) - BL_total + FC_total
    # Setting R_collected = R_target: s = (R_vol + BL_total) / (R_sample + BL_total)

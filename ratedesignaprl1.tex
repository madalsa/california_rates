...
The scaling factor $s$ is chosen so that the designed bills, \textit{including baseline credits}, collect exactly $\Rvol$ in volumetric revenue:
\begin{equation}
\sum_i \left[ \sum_p Q_i^p \cdot (s \cdot r^p) - \sum_{m=1}^{12} b \cdot \min(Q_i^m, B_i^m) \right] \times (1 - d_i) \times w = \Rvol
\label{eq:revenue_constraint}
\end{equation}

Solving for $s$:
\begin{equation}
\boxed{s = \frac{\Rvol + \BLtotal}{\sum_i \left[ \sum_p Q_i^p \cdot r^p \right] \times (1 - d_i) \times w} = \frac{\Rvol + \BLtotal}{R_{\text{gross\_vol}}}}
\label{eq:scaling}
\end{equation}

where $R_{\text{gross\_vol}} = \sum_i \left[\sum_p Q_i^p \cdot r^p\right] (1 - d_i) \cdot w$ is the gross volumetric revenue (TOU rates $\times$ consumption, with CARE discount, no baseline credits).

\subsection{Verification: Benchmark Scenario}

For F0\_WF0\_ROE0: $\Rtarget = \Rsample$, $\Rfixed = 0$, $\Rvol = \Rsample$.

From Equations~\eqref{eq:actual_bill} and \eqref{eq:r_sample}:
\[
\Rsample = R_{\text{gross\_vol}} - \BLtotal + \text{FC}_{\text{total}}
\]

Since F0\_WF0\_ROE0 has no designed fixed charges and the actual fixed charges are handled separately:
\[
s = \frac{\Rsample + \BLtotal}{R_{\text{gross\_vol}}} = \frac{(R_{\text{gross\_vol}} - \BLtotal) + \BLtotal}{R_{\text{gross\_vol}}} = 1.0
\]

Therefore, $\tilde{r}^p = r^p$ for all periods, and the designed bill equals the actual bill for every building. $\checkmark$

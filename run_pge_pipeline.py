...
UPGRADE11_DIR = './Upgrade11_PGE'  # Full electrification profiles (upgrade=11)
...
# Actual PGE tariff rate codes (computed via corrected_bill_calc.py)
ACTUAL_PGE_RATES = {
    'E-TOU-C': 'e_tou_c',        # Current TOU rate (no fixed charges)
    'E-TOU-C-F': 'e_tou_c_f',    # Current TOU rate with income-graduated fixed charges
}
...
# PGE TOU periods: 4 periods (NO midpeak)
# Summer: June–October (months 6-10)
# Peak: 4pm-9pm, Offpeak: all other hours
SUMMER_MONTHS = set(range(6, 11))  # June–October (1-indexed)

# PGE CEC climate zone representative coordinates for pvlib
# Keys are CEC climate zone integers found in PGE territory
PGE_CZ_COORDINATES = {
...
PGE_ANNUAL_KWH_PER_KW = 1600.0   # Default; updated at runtime from pvlib profiles
...
TOTAL_RESIDENTIAL_CUSTOMERS = 5_047_461  # EIA 861 bundled+unbundled
...
# ---------------------------------------------------------------------------
# Stage 2: Compute baseline bills for all PGE buildings
# ---------------------------------------------------------------------------

def get_tou_period(hour_of_day, month):
    """Classify hour into PGE TOU period (4 periods, no midpeak)."""
...
def build_tou_rate_array(scenario):
    """Build 8760-length rate array from a scenario row (4-period PGE)."""
    ...
    rates = np.where(
        is_summer,
        np.where(is_peak, scenario['summer_peak'], scenario['summer_offpeak']),
        np.where(is_peak, scenario['winter_peak'], scenario['winter_offpeak'])
    )
    return rates
...
def stage3_tech_assignments(bills_df):
    ...
    try:
        ...
        if os.path.exists(survey_path):
            ...
            renter_mask = pge_meta['own_bin'] == 0
            pv_scores[renter_mask] *= 0.15
            large_mf = pge_meta['in.geometry_building_type_acs'].isin(
...
# ---------------------------------------------------------------------------
# Stage 4: Generate solar profiles with pvlib (per CEC climate zone)
# ---------------------------------------------------------------------------
...
def stage4_solar_profiles(tech_df, bills_df):
    """
    Generate per-kW 8760 hourly solar profiles for each CEC climate zone in PGE territory.

    Unlike SDGE (single centroid), PGE covers 11 CEC climate zones.
    Returns a dict {cz: solar_per_kw_array} and a weighted-average annual kWh/kW.
    """
    global PGE_ANNUAL_KWH_PER_KW
    ...
    if 'in.cec_climate_zone' in tech_df.columns:
        ...
        pv_czs = sorted(PGE_CZ_COORDINATES.keys())
    ...
    for cz in pv_czs:
        cz_int = int(cz)
        if cz_int in PGE_CZ_COORDINATES:
            lat, lon, alt, name = PGE_CZ_COORDINATES[cz_int]
        else:
            # Fallback: use Sacramento
            lat, lon, alt, name = 38.55, -121.50, 10, f'CZ{cz_int}_fallback'
    ...
    # Compute weighted-average annual kWh/kW across all CZs
    if annual_kwh_per_kw_by_cz:
        avg_annual = np.mean(list(annual_kwh_per_kw_by_cz.values()))
        PGE_ANNUAL_KWH_PER_KW = avg_annual
        print(f"\n  Average annual generation: {avg_annual:,.0f} kWh/kW across {len(solar_profiles)} zones")

    return solar_profiles, annual_kwh_per_kw_by_cz


def _synthetic_solar_profile(latitude=37.75):
...
def stage5_battery_dispatch(hourly_load, solar_gen, rate_array, eec_rates=None):
    ...
    Uses scipy.optimize.linprog with sparse matrices.
    """
...
def stage6_post_adoption_bills(bills_df, tech_df, solar_profiles, rate_scenarios_df,
    ...
    if has_upgrade11:
        ...
        print(f"  WARNING: No Upgrade 11 data — S3/S4 scenarios will be skipped")
...
def stage7_summary(final_df, rate_scenarios_df):
    ...
    from rate_designer_pge import RESIDENTIAL_REVENUE, CUSTOMERS
...
def main():
    ...
    if args.skip_tech:
        ...
        if args.stage <= 4:
            solar_profiles, annual_kwh_per_kw_by_cz = stage4_solar_profiles(tech_df, bills_df)
        else:
        ...
        if args.stage <= 6:
            final_df = stage6_post_adoption_bills(
                ...
                use_lp=args.use_lp, annual_kwh_per_kw_by_cz=annual_kwh_per_kw_by_cz,
